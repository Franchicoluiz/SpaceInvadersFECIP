...
func _new_game():
    if (GameOver):  # Player dead or invaders have broken through 
        ...
        _reset_invaders()
        # Reinicia os timers de entrada das fileiras, igual a primeira onda.
        # Sem isto os invasores ficam congelados: _reset_invader_scene() faz
        # show() + Attack = true, mas o show() dispara screen_entered no
        # VisibleOnScreenNotifier2D, que zera Attack no frame seguinte.
        _set_off_invaders()
...
func _process(delta):
    ...
    if (!_game_over()):
        ...
        # Estado autoritativo e o do proprio Player: o PlayerAlive daqui so vira
        # false quando as vidas zeram, entao usa-lo deixava atirar durante a
        # explosao e o renascimento.
